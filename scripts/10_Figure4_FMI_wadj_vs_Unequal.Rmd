...
```{r}
sub_dat<-data2 %>% 
  dplyr::select(ISO3, prop_fish_fmi_wadj, disp_fished_fmi_wadj, FMI_weight_adj, med_diff_fmi_wadj, avg_diff_fmi_wadj) %>% 
...
```{r}
prop_fish_plot<-ggplot(sub_dat, aes(x = prop_fish_fmi_wadj , y = (FMI_weight_adj/100), label = ISO3, col = med_diff_fmi_wadj)) +
...
```{r}
ggsave(here("figures/Prop_fish_FMI_wadj_med_diff_plot.tiff"), dpi=300,  device = "tiff", width = 22, height=18, units = "cm")
```
...
```{r}
sub_dat2<-data2 %>% 
  dplyr::select(ISO3, prop_fish_fmi_wadj_coi, coi_disp_fished_fmi_wadj, FMI_weight_adj, med_diff_fmi_wadj, avg_diff_fmi_wadj) %>% 
  mutate(med_diff_fmi_wadj = ifelse(is.na(med_diff_fmi_wadj) == T, 0, as.numeric(med_diff_fmi_wadj)),
         avg_diff_fmi_wadj = ifelse(is.na(avg_diff_fmi_wadj) == T, 0, as.numeric(avg_diff_fmi_wadj))) %>% 
  filter(!is.na(FMI_weight_adj) == T)
...
```{r}
prop_fish_plot<-ggplot(sub_dat2, aes(x = prop_fish_fmi_wadj_coi , y = (FMI_weight_adj/100), label = ISO3))+#, col = avg_diff_fmi_wadj)) +
...
```{r}
ggsave(here("figures/Prop_fish_FMI_wadj_plot_coi.tiff"), dpi=300,  device = "tiff", width = 22, height=18, units = "cm")
```
