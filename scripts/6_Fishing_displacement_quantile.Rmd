...
Divide ME scores into 5 groups - expect that the more groups the more unequal displacement
1. High ME
2. Med High ME
3. Med ME
4. Med Low ME
...
```{r}
high_seas<-data.frame(X = NA, Export_ISO3 = "HighSeas", ME_score = 49)
...
me$quantile<-cutr::smart_cut(me$ME_score, 5, "g", output = "numeric")
...
Displacement (fishing) from countries with lower management by quantile

5 = highest management
1 = lowest management
...
unequal<-fish_me %>% 
  ...
  rename(Fished_ME = ME_score.x,
         ...
         Fished_quant = quantile.x,
         Fishing_quant = quantile.y) 

sub_unequal<-unequal %>% 
  filter(Fished_quant < Fishing_quant)
...
rev_hs<-fish_me %>% 
  ...
  rename(Fished_ME = ME_score.x,
         Fishing_ME = ME_score.y,
         Fished_quant = ME_score.x,
         Fishing_quant = ME_score.y) 
...
test<-fish_results %>% 
  filter(Fishing_ISO3 == "BEL") %>% 
  mutate(test = disp_fished - unequal_fish)
```
...
fish_results_country<-full_join(fishing_iso3, fish_displaced) %>% 
  ...
  rename(Fishing_ME = ME_score,
         Fishing_quant = quantile) %>% 
  full_join(., me, by = c("Fished_ISO3" = "Export_ISO3")) %>% 
  dplyr::select(-X) %>% 
  rename(Fished_ME = ME_score,
         Fished_quant = quantile)

test2<-fish_results_country %>% 
  filter(Fishing_ISO3 == "BEL",
         Year == 1976) %>% 
  mutate(test = disp_fished - unequal_disp_fished)
```



```{r}
write.csv(fish_results, here("output_data/Fish_displacement_quantile.csv"))

write.csv(fish_results_country, here("output_data/Fish_displacement_by_country_quantile.csv"))
```
