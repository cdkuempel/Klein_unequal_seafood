...
prop_fish_plot<-ggplot(sub_dat, aes(x = prop_fish , y = (FMI/100), label = labels, col = med_diff_fmi)) +
...
```{r}
ggsave(here("figures/Prop_fish_FMI_plot.tiff"), dpi=300,  device = "tiff", width = 18, height=18, units = "cm")
```
...
```{r}
prop_fish_wadj_plot<-ggplot(sub_dat_wadj, aes(x = prop_fish , y = (FMI_weight_adj/100), label = labels,col = med_diff_fmi_wadj)) +
  geom_point(aes(size = disp_fished/1000000)) +
  geom_smooth(method = "lm", se=FALSE, color="darkgrey", linetype = "dashed", formula = y~x) +
  xlab("Proportion of displaced seafood from\nless-effectively managed places") +
...
```{r}
prop_fish_wadj_plot2<-prop_fish_wadj_plot + geom_text(x = 0.10, y = 0.75, label = lm_eqn2(fish_wadj_plot), parse = TRUE, size = 8.5) 

prop_fish_wadj_plot2
```

```{r}
ggsave(here("figures/Prop_fish_FMI_wadj_med_color_plot.tiff"), dpi=300,  device = "tiff", width = 18, height=18, units = "cm")
```
