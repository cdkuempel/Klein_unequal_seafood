...
prop_fish_plot<-ggplot(sub_dat, aes(x = prop_fish , y = (FMI/100), label = labels))+#, col = avg_diff_fmi)) +
...
```{r}
prop_fish_wadj_plot<-ggplot(sub_dat_wadj, aes(x = prop_fish , y = (FMI_weight_adj/100), label = labels))+ #,col = avg_diff_fmi_wadj)) +
  geom_point(aes(size = disp_fished/1000000)) +
  geom_smooth(method = "lm", se=FALSE, color="darkgrey", linetype = "dashed", formula = y~x) +
  xlab("Proportion of displaced seafood from\nless-effectively managed places") +
