...
fmi<-readRDS(here("output_data/FMI_clean.Rdata")) %>% 
  dplyr::select(Country, Export_ISO3, FMI)
...
prop_fish_plot<-ggplot(sub_dat, aes(x = prop_fish , y = (FMI/100), label = labels)) +
...
prop_fish_wadj_plot<-ggplot(sub_dat_wadj, aes(x = prop_fish , y = (FMI/100), label = labels)) +
  geom_point(aes(size = disp_fished/1000000)) +
  geom_smooth(method = "lm", se=FALSE, color="darkgrey", linetype = "dashed", formula = y~x) +
  xlab("Proportion of displaced seafood from\nless-effectively managed places") +
  ylab("FMI") +
...
```{r}
ggsave(here("figures/Prop_fish_FMI_wadj_plot.tiff"), dpi=300,  device = "tiff", width = 18, height=18, units = "cm")
```
