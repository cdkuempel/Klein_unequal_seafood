...
```{r}
sub_dat<-data2 %>% 
  dplyr::select(ISO3, prop_fish_fmi, disp_fished_fmi, FMI, med_diff_fmi, avg_diff_fmi) %>% 
  mutate(med_diff_fmi = ifelse(is.na(med_diff_fmi) == T, 0, as.numeric(med_diff_fmi)),
         avg_diff_fmi = ifelse(is.na(avg_diff_fmi) == T, 0, as.numeric(avg_diff_fmi))) %>% 
  filter(!is.na(FMI) == T)

```

```{r}
prop_fish_plot<-ggplot(sub_dat, aes(x = prop_fish_fmi , y = (FMI/100), label = ISO3))+#, col = avg_diff_fmi)) +
  geom_point(aes(size = disp_fished_fmi/1000000)) +
...
```{r}
sub_dat2<-data2 %>% 
  dplyr::select(ISO3, prop_fish_fmi_coi, coi_disp_fished_fmi, FMI, med_diff_fmi, avg_diff_fmi) %>% 
  mutate(med_diff_fmi = ifelse(is.na(med_diff_fmi) == T, 0, as.numeric(med_diff_fmi)),
         avg_diff_fmi = ifelse(is.na(avg_diff_fmi) == T, 0, as.numeric(avg_diff_fmi))) %>% 
  filter(!is.na(FMI) == T)
...
```{r}
prop_fish_plot<-ggplot(sub_dat2, aes(x = prop_fish_fmi_coi , y = (FMI/100), label = ISO3))+#, col = avg_diff_fmi)) +
  geom_point(aes(size = coi_disp_fished_fmi/1000000)) +
  geom_smooth(method = "lm", se=FALSE, color="darkgrey", linetype = "dashed", formula = y~x) +
  xlab("Proportion of displaced seafood from\nless-effectively managed places") +
  ylab("FMI") +
  labs(size = "Total Displaced\n(Millions of Tonnes)",
...
```{r}
ggsave(here("figures/Prop_fish_FMI_plot_coi.tiff"), dpi=300,  device = "tiff", width = 22, height=18, units = "cm")
```
