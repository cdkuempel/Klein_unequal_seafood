...
me<-readRDS(here("output_data/FMI_clean.Rdata")) %>% 
  dplyr::select(Export_ISO3, FMI_weight_adj) %>% 
  rename(ME_score = FMI_weight_adj)
```
...
Checking why we end up with only 17 countries, and why China does not have unequal displacement. 
```{r}
test<-fish_results_country %>% 
  filter(is.na(Fishing_ME) == F)

length(unique(test$Fishing_ISO3))

test2<-test %>% 
  filter(is.na(disp_fished) == F)

length(unique(test2$Fishing_ISO3))

test3<-test2 %>% 
  filter(is.na(unequal_disp_fished) == F)

length(unique(test3$Fishing_ISO3))
...
chn<-fish_results_country %>% 
  filter(Fishing_ISO3 == "CHN")

sub_chn<-chn %>% 
  filter(Fishing_ME>Fished_ME)
```



```{r}
write_csv(fish_results, here("output_data/Fish_displacement_FMI_wadj.csv"))

write_csv(fish_results_country, here("output_data/Fish_displacement_by_country_FMI_wadj.csv"))
```